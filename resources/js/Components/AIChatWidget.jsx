...
function MenuCard({ item }) {
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
            <Link href={route('restaurants.show', item.restaurant_id)} className="flex items-center gap-2.5 p-2.5 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400 truncate">{item.restaurant_name} · {item.municipality}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                        <span className="text-xs font-extrabold text-green-600">₱{Number(item.price).toFixed(0)}</span>
                        {item.category && <span className="text-[9px] text-gray-300">{item.category}</span>}
                    </div>
                </div>
            </Link>
            <div className="border-t border-gray-50 px-2.5 py-2">
                {status === 'error' ? (
                    <p className="text-[10px] text-red-500 text-center font-semibold">Clear your cart first (different restaurant)</p>
                ) : (
                    <button
                        onClick={addToCart}
...
function VoucherCard({ voucher }) {
    const [status, setStatus] = useState(
        voucher.is_used ? 'used' : voucher.is_claimed ? 'claimed' : 'idle'
    );
    ...
    async function claimVoucher() {
        ...
        setStatus('claiming');
        try {
            const res = await fetch(route('vouchers.claim'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken() },
                body: JSON.stringify({ voucher_id: voucher.id }),
            });
            const data = await res.json();
            setStatus(res.ok && data.claimed ? 'claimed' : 'idle');
        } catch {
            setStatus('idle');
        }
    }
    ...
    const discountLabel = voucher.type === 'percentage'
        ? `${Number(voucher.value).toFixed(0)}% off`
        : `₱${Number(voucher.value).toFixed(0)} off`;

    return (
        <div className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-100 rounded-xl p-2.5">
            <div className="flex items-start gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-orange-500 text-white flex items-center justify-center text-[11px] font-extrabold shrink-0">
                    {voucher.type === 'percentage' ? '%' : '₱'}
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-xs font-extrabold text-gray-800">{discountLabel}</span>
                        {voucher.restaurant_name
                            ? <span className="text-[9px] text-orange-600 font-semibold">at {voucher.restaurant_name}</span>
                            : <span className="text-[9px] text-green-600 font-semibold">All restaurants</span>
                        }
                    </div>
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        <button
                            onClick={copyCode}
                            className="font-mono text-[10px] font-bold text-gray-600 bg-white border border-gray-200 px-1.5 py-0.5 rounded-md hover:border-orange-300 hover:text-orange-600 transition-colors"
                            title="Copy code"
                        >
                            {voucher.code}
                        </button>
                        {voucher.min_order_amount && (
                            <span className="text-[9px] text-gray-400">Min. ₱{Number(voucher.min_order_amount).toFixed(0)}</span>
                        )}
                        {voucher.expires_at && (
                            <span className="text-[9px] text-gray-400">Exp. {voucher.expires_at}</span>
                        )}
                    </div>
                </div>
            </div>
            <button
                ...
                disabled={status !== 'idle'}
                className={`w-full py-1.5 rounded-lg text-[11px] font-bold transition-all ${
                    status === 'claimed' ? 'bg-green-50 text-green-600 border border-green-100'
                    : status === 'used' ? 'bg-gray-50 text-gray-400 border border-gray-100 cursor-not-allowed'
                    : 'bg-orange-500 text-white hover:bg-orange-600'
                } ${status === 'claiming' ? 'opacity-60' : ''}`}
            >
                {status === 'claimed' ? '✓ Claimed — use at checkout'
...
function MessageBubble({ message }) {
    ...
    const hasMenuCards    = !isUser && message.menu_cards?.length > 0;
    const hasVoucherCards = !isUser && message.voucher_cards?.length > 0;

    return (
        <div className={`flex items-start gap-2.5 ${isUser ? 'flex-row-reverse ml-auto max-w-[85%]' : 'max-w-[92%]'}`}>
            {!isUser && (
                <div className="w-7 h-7 rounded-full bg-green-500 flex items-center justify-center shrink-0 mt-0.5">
                    <SparkleIcon className="h-3.5 w-3.5 text-white" />
                </div>
            ...
            <div className="flex-1 min-w-0">
                ...
                {hasMenuCards && (
                    <div className="mt-2 space-y-2">
                        <p className="text-[10px] text-gray-400 font-semibold ml-1">Recommended dishes</p>
                        {message.menu_cards.map(item => (
                ...
                {hasVoucherCards && (
                    <div className="mt-2 space-y-2">
                        <p className="text-[10px] text-gray-400 font-semibold ml-1">Available promos</p>
                        {message.voucher_cards.map(voucher => (
...
const DEFAULT_SUGGESTIONS     = ['Masarap na sabaw', 'Something sweet', 'Budget meal under ₱100', 'Any promos?'];
const RESTAURANT_SUGGESTIONS  = ["What's popular here?", 'Best value meal?', 'Something light', 'Your top pick?'];
...
export default function AIChatWidget({ restaurantId = null, restaurantName = null }) {
    ...
    const subtitle    = restaurantName ? `Ask me about ${restaurantName}'s menu` : "Tell me what you're craving";
        ...
        try {
            ...
            if (res.ok && data.reply) {
                setMessages([...updatedMessages, {
                    role:          'assistant',
                    content:       data.reply,
                    menu_cards:    data.menu_cards    ?? [],
                    voucher_cards: data.voucher_cards ?? [],
                }]);
            } else if (res.status === 503) {
                setError('Hapag AI is a bit busy right now. Try again in a sec!');
            } else {
    ...
    const handleSuggestion = (t) => { setInput(''); sendMessage(t); };
    const handleClearChat  = () => { setMessages([]); setError(''); };
                ...
                <div className="fixed bottom-24 right-6 z-40 w-[390px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col"
                    ...
                    <div className="bg-gradient-to-r from-green-500 to-green-600 px-4 py-3.5 shrink-0">
                        <div className="flex items-center justify-between">
                            ...
                            {hasMessages && (
                                <button onClick={handleClearChat} className="text-white/50 hover:text-white/90 p-1.5 rounded-lg hover:bg-white/10 transition-colors" title="New conversation">
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-center px-2">
                                ...
                                <p className="text-gray-400 text-xs mb-5 leading-relaxed max-w-[240px]">
                                    {restaurantName
                                        ...
                                        : 'I know every restaurant on Hapag. Ask me anything!'}
                                </p>
                    ...
                    <form onSubmit={handleSubmit} className="border-t border-gray-100 p-3 flex gap-2 shrink-0">
                        ...
                        <button type="submit" disabled={loading || !input.trim()}
                            className="w-10 h-10 rounded-xl bg-green-500 text-white flex items-center justify-center hover:bg-green-600 disabled:opacity-40 transition-all shrink-0">
